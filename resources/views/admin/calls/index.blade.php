        ...
        <table class="w-full">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-200">
                    ...
                    <th class="text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500 px-5 py-3.5">Direction</th>
                    <th class="text-left text-[11px] font-semibold uppercase tracking-wider text-gray-500 px-5 py-3.5">Duration</th>
            ...
            <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-5 py-4">
                            <a href="{{ route('admin.users.show', $call->user) }}" class="flex items-center gap-3 group">
                                ...
                                <div class="min-w-0">
                                    <div class="text-sm font-semibold text-gray-900 group-hover:text-primary transition-colors truncate">{{ $call->user?->name ?? 'Deleted User' }}</div>
                        ...
                        <td class="px-5 py-4 text-sm text-gray-500">{{ $call->created_at->format('M d, Y h:i A') }}</td>
                        <td class="px-5 py-4 text-right">
                            <a href="{{ route('admin.calls.show', $call) }}" class="inline-flex items-center gap-1.5 text-xs font-semibold text-primary hover:text-primary-container transition-colors">
                                View Details
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-5 py-8 text-center text-sm text-gray-500">No calls found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>

...
<body
    class="bg-background text-on-background font-body-md selection:bg-primary-fixed selection:text-on-primary-fixed min-h-screen flex flex-col"
    x-data="{ waitlistModalOpen: false, waitlistSubmitted: false, email: '' }">
    <!-- TopNavBar -->
    <nav
        class="docked full-width top-0 z-50 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 shadow-sm fixed w-full">
        <div
            class="flex justify-between items-center h-16 px-6 md:px-12 max-w-7xl mx-auto font-inter antialiased tracking-tight">
            <a href="{{ url('/') }}"
                class="font-black tracking-tighter text-slate-900 dark:text-white flex items-baseline">
            ...
            <div class="hidden md:flex items-center space-x-8">
                <a class="text-blue-600 dark:text-blue-400 font-semibold border-b-2 border-blue-600 py-1"
                    href="{{ url('/companions') }}">Product</a>
                <a class="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 transition-colors py-1"
                    href="#">Features</a>
            </div>
            <div class="flex items-center gap-4">
                <a href="{{ url('/login') }}"
                    class="text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-900 transition-all duration-200 px-4 py-2 rounded-lg text-sm font-medium">Sign
                    In</a>
                <button @click="waitlistModalOpen = true"
                    class="bg-primary hover:bg-primary-container text-white active:scale-95 transition-all duration-200 px-5 py-2 rounded-lg text-sm font-semibold">Join
                    Waitlist</button>
            </div>
        </div>
    </nav>
    ...
    <!-- Footer -->
    <footer
        class="full-width py-12 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 mt-auto">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 px-6 md:px-12 max-w-7xl mx-auto font-inter">
            <div class="space-y-4">
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 max-w-xs">
                    Revolutionizing professional productivity through contextual intelligence and voice-first
                    interaction.
                </p>
            </div>
            <div
                class="flex flex-col md:flex-row justify-between items-center gap-4 pt-8 mt-8 border-t border-slate-200 dark:border-slate-800 md:col-span-2">
                <div class="text-sm text-slate-500 dark:text-slate-400">© {{ date('Y') }} The Business Companion AI.
                    Engineered for Excellence.</div>
            </div>
        </div>
    </footer>
    <!-- Waitlist Modal -->
        ...
        <div class="bg-white rounded-[2.5rem] p-8 md:p-12 max-w-lg w-full shadow-2xl relative border border-gray-100"
            ...
            <div x-show="!waitlistSubmitted">
                <div class="mb-8">
                    ...
                    <p class="text-gray-500">We are currently in private beta. Leave your email to get early access when
                        we
                        expand.
                    </p>
                </div>

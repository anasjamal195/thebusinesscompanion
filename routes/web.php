...
Route::middleware("guest")->group(function () {
    ...
    Route::get("/auth/google/redirect", [
        GoogleSocialiteController::class,
        "redirect",
...
Route::middleware("auth")->group(function () {
    // Onboarding
    Route::get("/onboarding/schedule", [
        OnboardingController::class,
        "schedule",
    ])->name("onboarding.schedule");
    Route::post("/onboarding/schedule", [
        OnboardingController::class,
        "saveSchedule",
    ])->name("onboarding.schedule.save");

    // Dashboard
    Route::get("/dashboard", [ProjectController::class, "index"])->name(
        "dashboard",
    );
    ...
    Route::post("/tasks", [TaskController::class, "store"])->name(
        "tasks.store",
    );
    ...
    Route::post("/tasks/mark-day-completed", [
        TaskController::class,
        "markDayCompleted",
    ])->name("tasks.mark-day-completed");
    ...
    Route::get("/calls/{call}", [CallController::class, "show"])->name(
        "calls.show",
    );
    Route::get("/calls/{call}/transcript", [
        CallController::class,
        "downloadTranscript",
    ])->name("calls.transcript");
    Route::post("/calls/request", [CallController::class, "requestCall"])->name(
        "calls.request",
    );
    ...
    Route::get("/reports/{report}/pdf", [ReportController::class, "pdf"])->name(
        "reports.pdf",
    );
    Route::post("/reports/daily", [
    ...
    Route::get("/calendar/data", [CalendarController::class, "data"])->name(
        "calendar.data",
    );

    // Shared Report to Post
    Route::post("/reports/{dailyReport}/share", [
        ReportController::class,
        "shareReport",
    ])->name("reports.share");

    // Community (Feed only)
    Route::get("/community", [CommunityController::class, "feed"])->name(
        "community.feed",
    );
    ...
    Route::put("/community/{post}", [
        CommunityController::class,
        "updatePost",
    ])->name("community.posts.update");
    Route::post("/community/{post}/delete", [
        CommunityController::class,
        "destroyPost",
    ...
    Route::post("/community/{post}/comment", [
        CommunityController::class,
        "comment",
    ])->name("community.comment");
    Route::post("/community/follow/{user}", [
        CommunityController::class,
        "follow",
    ...
    // Followers
    Route::get("/followers", [
        \App\Http\Controllers\FollowController::class,
        "followers",
    ])->name("followers.index");
    Route::post("/followers/{user}/remove", [
        \App\Http\Controllers\FollowController::class,
        "removeFollower",
    ])->name("followers.remove");
    Route::post("/followers/{follow}/accept", [
        \App\Http\Controllers\FollowController::class,
        "acceptFollow",
    ])->name("followers.accept");
    Route::post("/followers/{follow}/reject", [
        \App\Http\Controllers\FollowController::class,
        "rejectFollow",
    ])->name("followers.reject");
});

// ── Voice Previews ────────────────────────────────────────────────────────────

Route::get("/api/voice-preview/{voiceId}", VoicePreviewController::class)->name(
    "voice.preview",
);
...
Route::middleware(["auth", "admin"])
    ...
    ->group(function () {
        ...
        Route::get("/calls/{call}", [AdminCallController::class, "show"])->name(
            "calls.show",
        );
        Route::get("/payments", [AdminPaymentController::class, "index"])->name(
            "payments.index",
        );
        Route::get("/monetization", [
            AdminMonetizationController::class,
            "index",
        ])->name("monetization.index");
        Route::post("/monetization", [
            AdminMonetizationController::class,
            "update",
        ])->name("monetization.update");
        Route::get("/waitlist", [
            AdminWaitlistController::class,
            "index",
        ])->name("waitlist.index");
        Route::delete("/waitlist/{entry}", [
            AdminWaitlistController::class,
            "destroy",
...
Route::post("/vapi/webhook", [
    \App\Http\Controllers\VapiWebhookController::class,
    "handle",
])->name("vapi.webhook");
